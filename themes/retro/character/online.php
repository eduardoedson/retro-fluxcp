...
<?php if ($auth->allowedToSearchWhosOnline): ?>
    ...
    <form action="<?php echo $this->url ?>" method="get" class="search-form">
        ...
        <p>
            ...
            <input type="text" name="char_class" id="char_class"
                   value="<?php echo htmlspecialchars($params->get('char_class') ?: '') ?>"/>
            ...
            <label for="guild_name">Guild:</label>
            <input type="text" name="guild_name" id="guild_name"
                   value="<?php echo htmlspecialchars($params->get('guild_name') ?: '') ?>"/>
...
<?php if ($chars): ?>
    ...
    <?php if ($hiddenCount): ?>
        <p><?php echo number_format($hiddenCount) ?> <?php echo ((int)$hiddenCount === 1) ? 'person has' : 'people have' ?>
            chosen to hide themselves from this list.</p>
    <?php endif ?>
    ...
    <table class="horizontal-table">
        ...
        <?php foreach ($chars as $char): ?>
            <tr>
                ...
                <?php else: ?>
                    <td colspan="2"><span class="not-applicable">-</span></td>
                <?php endif ?>
...
<?php else: ?>
    <p>No characters found on <?php echo htmlspecialchars($server->serverName) ?>. <a href="javascript:history.go(-1)">Go
            back</a>.</p>
<?php endif ?>
